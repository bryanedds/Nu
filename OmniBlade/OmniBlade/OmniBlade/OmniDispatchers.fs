...
type OmniFieldGroupDispatcher () =
    ...
    let adjustFieldCamera groupAddress world =
        ...
        let entity = get world <| EntityModule.worldEntityLens avatarAddress
    ...
    let moveFieldAvatarHandler groupAddress _ _ message world =
        ...
        let feeler = get world <| EntityModule.worldEntityLens feelerAddress
        if feeler?IsTouched () then
            let avatarAddress = groupAddress @ [OmniConstants.FieldAvatarName]
            let avatar = get world <| EntityModule.worldEntityLens avatarAddress
            let camera = world.Camera
            let view = CameraModule.getInverseViewF camera
    ...
    override this.Register (address, omniBattleGroup, entities, world) =
        let world_ = WorldModule.subscribe Constants.TickAddress [] (moveFieldAvatarHandler address) world
        let world_ = WorldModule.subscribe Constants.TickAddress [] (adjustFieldCameraHandler address) world_
    ...
    override this.Unregister (address, omniFieldGroup, world) =
        let world_ = WorldModule.unsubscribe Constants.TickAddress [] world
        let world_ = WorldModule.unsubscribe Constants.TickAddress [] world_
