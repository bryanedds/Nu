...
type [<ReferenceEquality>] Observation<'a, 'o, 'w when 'o :> Addressable and 'w :> 'w Eventable> =
    { Observer : 'o
...
module Observation =
    ...
    let [<DebuggerHidden; DebuggerStepThrough>] observe<'a, 'o, 'w when 'o :> Addressable and 'w :> 'w Eventable>
        ...
        let subscribe = fun (world : 'w) ->
            ...
            let unsubscribe = fun world -> EventSystem.unsubscribe subscriptionKey world
            let subscription = fun evt world ->
                let eventTrace = "Observation.observe" :: evt.Trace
                let world = World.publish5<'a, Addressable> World.sortSubscriptionsNone evt.Data subscriptionAddress eventTrace evt.Publisher world
                (Cascade, world)
        ...
        let subscribe = fun world ->
            ...
            let subscription = fun evt world ->
                let subscription' = fun event' world ->
                    ...
                    let world = World.publish5<'a * 'b, Addressable> World.sortSubscriptionsNone eventData subscriptionAddress'' eventTrace evt.Publisher world
                    let world = World.unsubscribe subscriptionKey' world
                    (Cascade, world)
    ...
    let [<DebuggerHidden; DebuggerStepThrough>] sum (eventAddress : 'b Address) (observation : Observation<'a, 'o>) : Observation<Either<'a, 'b>, 'o> =
        let subscribe = fun world ->
            ...
            let subscription = fun evt world ->
                ...
                let world = World.publish5<Either<'a, 'b>, Addressable> World.sortSubscriptionsNone eventData subscriptionAddress'' eventTrace evt.Publisher world
                (Cascade, world)
            let subscription' = fun evt world ->
                let eventTrace = "Observation.sum" :: evt.Trace
            ...
            let subscription = fun evt world ->
                let world =
                    if pred evt world then
                        ...
                        World.publish5<'a, Addressable> World.sortSubscriptionsNone evt.Data subscriptionAddress eventTrace evt.Publisher world
                    else world
                (Cascade, world)
    ...
    let [<DebuggerHidden; DebuggerStepThrough>] map (mapper : Event<'a, 'o> -> World -> 'b) (observation : Observation<'a, 'o>) : Observation<'b, 'o> =
        let subscribe = fun world ->
            ...
            let subscription = fun evt world ->
                ...
                let world = World.publish5<'b, Addressable> World.sortSubscriptionsNone (mapper evt world) subscriptionAddress eventTrace evt.Publisher world
                (Cascade, world)
            let world = World.subscribe5<'a, 'o> subscriptionKey subscription eventAddress observation.Observer world
            (subscriptionAddress, unsubscribe, world)
        { Observer = observation.Observer; Subscribe = subscribe }
            ...
            let subscription = fun evt world ->
                ...
                let world =
                    if tracked then
                        ...
                        World.publish5<'b, Addressable> World.sortSubscriptionsNone eventData subscriptionAddress eventTrace evt.Publisher world
                    else world
                (Cascade, world)
            let world = World.subscribe5<'a, 'o> subscriptionKey subscription eventAddress observation.Observer world
            (subscriptionAddress, unsubscribe, world)
            ...
            let subscription = fun evt world ->
                ...
                let world =
                    if tracked then
                        ...
                        World.publish5<'a, Addressable> World.sortSubscriptionsNone state subscriptionAddress eventTrace evt.Publisher world
                    else world
                (Cascade, world)
            let world = World.subscribe5<'a, 'o> subscriptionKey subscription eventAddress observation.Observer world
            (subscriptionAddress, unsubscribe, world)
