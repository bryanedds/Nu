...
module EvaluatorPrimitives =
        ...
        let optEntry =
            List.tryFindPlus
                (fun (frame : ProceduralFrame) ->
                    ...
                    refOptIndex := Array.tryFindIndexRev (fun (entryName, _) -> name $ entryName) frame // OPTIMIZATION: ($) is faster than (=) here
    ...
    let tryFindEntry name env =
        let optEntry =
            List.tryFindPlus
                (fun (frame : ProceduralFrame) ->
                    let optEntry = Array.tryFind (fun (entryName, _) -> name $ entryName) frame // OPTIMIZATION: ($) is faster than (=) here
    ...
    let hasType typeName value env =
        ...
        match vtype with
        | Composite composite -> composite.CompName $ typeName // OPTIMIZATION: ($) is faster than (=) here
        | _ -> false
    ...
    module SignatureValidationModule =
        ...
        let tryPartialUnifyArgs (argsEvaluated : bool) (args1 : Expr list) (largs1 : Arg list) larg =
            ...
            | Labeled ->
                ...
                | Package package ->
                    if package.PkgName $ larg.ArgName then // OPTIMIZATION: ($) is faster than (=) here
                        let unifiedArg = Some (package.PkgExpr, larg)
                        makePartialUnifyResult unifiedArg args1.Tail 0 true
                    else
                        let matchingLargIndex = List.findIndex (fun larg -> larg.ArgName $ package.PkgName) largs1 // OPTIMIZATION: ($) is faster than (=) here
        ...
        let projectConstraintToProtocolArg (instances, matches) ((constr, parg : string) as instance) =
            ...
            | Some prevConstraint ->
                if prevConstraint.ConstrName $ constr.ConstrName then ((true, instance) :: instances, matches) // OPTIMIZATION: ($) is faster than (=) here
