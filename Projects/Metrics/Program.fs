        ...
        let positions = // 37,500 entity positions (goal: 60FPS, current: 59FPS)
            [|for i in 0 .. dec 50 do
                for j in 0 .. dec 50 do
                    for k in 0 .. dec 50 do
                        yield v3 (single i * 0.4f) (single j * 0.4f) (single k * 0.5f)|]
        let world =
            Array.fold (fun world position ->
                let (entity, world) = World.createEntity<StaticModelDispatcher> NoOverlay (Some [|string Gen.id64|]) group world
                let world = entity.SetPresence Omnipresent world
                let world = entity.SetPosition (position + v3 -10.0f -10.0f -30.0f) world
