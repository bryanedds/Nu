...
#if !ECS_PURE
        let positions = // 15,125 entity positions (goal: 60FPS)
            seq {
                for i in 0 .. 54 do
                    for j in 0 .. 54 do
                        for k in 0 .. 4 do
            ...
            [Content.group Gen.name []
                [Content.entity<ElmishEntityDispatcher> Gen.name
                    (seq {
                        ...
                        for index in 0 .. 24999 do yield Entity.Size <== int --> fun int -> v2 (single (int % 12)) (single (index % 12)) } |> // 25,000 property bindings (goal: 60FPS)
                        Seq.toList)]
             Content.group Gen.name []
...
type ElmishGameDispatcher () =
    inherit GameDispatcher<Intss, int, unit> (Intss.init 70) // 4900 ints (goal: 60FPS)
