...
type ElmishGameDispatcher () =
    inherit GameDispatcher<Intss, int, unit> (Intss.init 1) // 4900 ints (goal: 60FPS)
                    ...
                    [Content.entities intss (fun ints _ -> ints.Ints) constant (fun j int _ ->
                        Content.entity<ElmishEntityDispatcher> (string j)
                            (seq {
                                ...
                                for _ in 0 .. 20000 do yield Entity.Size <== int --> fun int -> v2 (single (int % 12)) (single (int % 12)) } |>
                                Seq.toList))])
             Content.group "Group" []
