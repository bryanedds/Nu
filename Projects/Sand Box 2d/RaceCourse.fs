...
type RaceCourseDispatcher () =
    ...
    static let RaceCourseScale = Geometrics.RaceCourseScale
    static let CarMotorSpeedMax = Geometrics.CarMotorSpeedMax

    static let RaceTrackPoints =
        Geometrics.RaceTrackContour
        |> Array.map ((*) RaceCourseScale)
        |> Array.map _.V3

    static let CarContour = Geometrics.CarContour
    static let CarContourBounds = Geometrics.CarContourBounds
    ...
    static let CarSize =
        Geometrics.carSize.V3

    static let CarSpawnPosition =
        (Geometrics.CarContourBounds.Center * RaceCourseScale).V3 + v3 0f RaceCourseScale 0f
        ...
        if raceCourse.GetSelected world then
            ...
            World.doBoxBody2d "Car"
                [Entity.Position |= CarSpawnPosition
                 ...
                 Entity.Substance .= Density (Geometrics.CarChassisDensity * Geometrics.SourceMassScale)
                 Entity.Friction .= Geometrics.CarChassisFriction] world |> ignore
            let car = world.DeclaredEntity

            // declare wheels (and joints)
            for (relation, position, density, frequency, friction, maxTorque, isMotor) in Geometrics.CarWheelSpecs do
                ...
                World.doBallBody2d $"Wheel {relation}"
                    [Entity.Position |= wheelPosition
                     ...
                     Entity.Substance .= Density (density * Geometrics.SourceMassScale)
                ...
                let (bodyJointId, _) =
                    World.doBodyJoint2d $"Wheel {relation} Joint"
                        [Entity.BodyJoint |= Box2dNetBodyJoint { CreateBodyJoint = fun _ _ car wheel world ->
                            ...
                            jointDef.dampingRatio <- Geometrics.CarWheelDampingRatio
                            jointDef.maxMotorTorque <- maxTorque * Geometrics.SourceTorqueScale
                ...
                if raceCourse.GetSelected world && isMotor then
                    ...
                    let motorSpeed = Geometrics.carMotorSpeed acceleration
            ...
            let teeterBodyId, _ =
                World.doBoxBody2d "Teeter Board"
                    [Entity.Position |= v3 140f Geometrics.TeeterCenterY 0f * RaceCourseScale
                     Entity.Rotation |= Quaternion.CreateFromAngle2d Geometrics.TeeterInitialAngle
                     Entity.Size .= v3 (2f * Geometrics.TeeterBoardHalfLength) (2f * Geometrics.TeeterBoardHalfThickness) 0f * RaceCourseScale
                     Entity.StaticImage .= Assets.Default.Paddle
                     Entity.Substance .= Density Geometrics.SourceMassScale
            ...
            World.doBodyJoint2d "Teeter Joint"
                [Entity.BodyJoint |= Box2dNetBodyJoint { CreateBodyJoint = fun _ _ a b world ->
                    ...
                    jointDef.lowerAngle <- -Geometrics.TeeterAngleLimit
                    jointDef.upperAngle <- Geometrics.TeeterAngleLimit
            ...
            if teeterInitializing then
                World.applyBodyAngularImpulse (v3 0f 0f Geometrics.TeeterAngularImpulse) teeterBodyId world

            // declare bridge
            let bridgeEndpoints =
                Array.init Geometrics.BridgeLinkCount (fun i ->
                    v3 (160f + 2f * single i) -0.125f 0f, v3 (160f + 2f * single (i + 1)) -0.125f 0f)
                |> Array.map (fun (a, b) -> a * RaceCourseScale, b * RaceCourseScale)
            for i in 0 .. Geometrics.BridgeLinkCount do
                if i < Geometrics.BridgeLinkCount then
                    let endpoint1, endpoint2 = bridgeEndpoints[i]
                    World.doBoxBody2d $"Bridge {i}"
                        [Entity.Position |= (endpoint1 + endpoint2) / 2f
                         Entity.Rotation |= quatIdentity
                         Entity.Size .= v3 (endpoint2 - endpoint1).Magnitude Geometrics.BridgeLinkThickness 0f
                         Entity.Friction .= 0.6f
                         Entity.StaticImage .= Assets.Default.Paddle
                    ...
                    [Entity.BodyJoint |= Box2dNetBodyJoint {
                        CreateBodyJoint = fun _ toPhysicsV2 a b world ->
                            let p =
                                ...
                                elif i = Geometrics.BridgeLinkCount then toPhysicsV2 (snd bridgeEndpoints[i - 1])
                     ...
                     Entity.BodyJointTarget2 .= Address.makeFromString (if i < Geometrics.BridgeLinkCount then $"^/Bridge {i}" else "^/Race Track")
                     // Adjacent planks already touch at their joint anchors; avoid collision impulses fighting the hinge.
                     Entity.CollideConnected .= Geometrics.BridgeCollideConnected] world |> ignore
            ...
            World.setGravity2d (v3 0f Geometrics.SourceGravity 0f) world
