...
type AvatarDispatcher () =
    ...
    override this.Definitions (avatar, _) =
        let bodyShape =
            BodyShapes
                [SphereShape { Radius = 0.165f; TransformOpt = Some (Affine.makeTranslation (v3 -0.013f -0.356f 0.0f)); PropertiesOpt = Some { BodyShapeProperties.empty with BodyShapeIndex = Constants.Field.AvatarCollisionShapeIndex }}
                 SphereShape { Radius = 0.320f; TransformOpt = Some (Affine.makeTranslation (v3 -0.013f -0.356f 0.0f)); PropertiesOpt = Some { BodyShapeProperties.empty with BodyShapeIndex = Constants.Field.AvatarSensorShapeIndex; SensorOpt = Some true }}]
