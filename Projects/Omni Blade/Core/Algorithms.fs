...
module Algorithms =
    ...
    let expReqs =
        [0; 10; 25
         50; 100; 200; 400; 800; 1600 // 2x increase
         2400; 3600; 5400; 8100; 12150 // 1.5x increase
         14580; 17496; 20995; 25194; 30233 // 1.2x increase
         34768; 39983; 45980; 52877; 60809 // 1.15x increase
         66890; 73579; 80937; 89031; 97934] // 1.1x increase
    ...
    let affinityOpt accessories archetypeType (level : int) =
        ...
        let affinityOpt =
            ...
            | None -> None
        List.fold (fun affinityOpt accessoryType ->
    ...
    let magic isWindOrShadow weaponOpt statuses archetypeType level =
        let magicBuff =
            if isWindOrShadow then // wind and shadow are affected by power buffs
                ...
                Option.mapOrDefaultValue (function Magic (false, false) -> 0.5f | Magic (false, true) -> 0.25f | Magic (true, false) -> 1.5f | Magic (true, true) -> 2.0f | _ -> 1.0f) 1.0f
        let intelligence = 
            match Map.tryFind archetypeType Data.Value.Archetypes with
    ...
    let shield effectType accessories statuses archetypeType level =
        ...
        let (defense, absorb) = 
            ...
            | None -> (1.0f, 1.0f)
        let shieldBase =
            List.fold (fun shieldBase accessoryType ->
                match Map.tryFind accessoryType Data.Value.Accessories with
