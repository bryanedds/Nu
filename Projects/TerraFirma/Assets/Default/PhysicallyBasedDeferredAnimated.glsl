...
void main()
{
    ...
    float height_ = texture(heightTexture, texCoordsOut).r * height;
    vec2 parallax = toEyeTangent.xy * height_;
    ...
    albedo_ = pow(albedoSample.rgb, vec3(GAMMA)) * albedo.rgb;
    ...
    float emission = texture(emissionTexture, texCoords).r * material.a;
    material_ = vec4(roughness, metallic, ambientOcclusion, emission);

    // compute normal and height
    normalAndHeight.xyz = normalize(toWorld * (texture(normalTexture, texCoords).xyz * 2.0 - 1.0));
    normalAndHeight.a = height_;
}
