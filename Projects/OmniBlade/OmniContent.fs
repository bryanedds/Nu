...
module Content =
    ...
    let sidebar name position elevation (field : Field) menuTeamOpen menuItemsOpen menuTechOpen menuOptionsOpen menuClose =
    ...
    let team (position : Vector3) elevation rows (field : Field) filter fieldMsg =
    ...
    let techs (position : Vector3) elevation (field : Field) fieldMsg =
        let techs =
            match field.Menu.MenuState with
            | MenuTech menuTech ->
