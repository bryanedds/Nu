...
vec4 depthToPosition(float depth, vec2 texCoords)
{
    ...
    vec4 positionView = commonData.projectionInverse * positionClip;
    positionView /= positionView.w;
    return commonData.viewInverse * positionView;
}
...
{
    vec3 directionWorld = positionWorld - commonData.eyeCenter;
...
float computeShadowScalarPoint(vec4 position, vec3 lightOrigin, int shadowIndex)
{
    ...
    for (int i = 0; i < commonData.lightShadowSamples; ++i)
    {
        for (int j = 0; j < commonData.lightShadowSamples; ++j)
        {
            for (int k = 0; k < commonData.lightShadowSamples; ++k)
            {
                vec3 offset = (vec3(i, j, k) - vec3(commonData.lightShadowSamples / 2.0)) * (commonData.lightShadowSampleScalar / commonData.lightShadowSamples);
                shadowHits += shadowZ - commonData.lightShadowBias > texture(samplerCube(shadowMaps[shadowIndex - SHADOW_TEXTURES_MAX], shadowSampler), positionShadow + offset).x ? 1.0 : 0.0;
            }
        }
    }
    return 1.0 - shadowHits / (commonData.lightShadowSamples * commonData.lightShadowSamples * commonData.lightShadowSamples);
}
...
float computeShadowScalarSpot(vec4 position, float lightConeOuter, int shadowIndex)
{
        ...
        shadowTexCoordsProj.z >= -1.0 && shadowTexCoordsProj.z < 1.0)
    {
        ...
        float shadowZExp = exp(-commonData.lightShadowExponent * shadowZ);
        ...
        shadowScalar = pow(shadowScalar, commonData.lightShadowDensity);
...
float computeShadowScalarCascaded(vec4 position, float shadowCutoff, int shadowIndex)
{
    for (int i = 0; i < SHADOW_CASCADE_LEVELS; ++i)
    {
            ...
            shadowTexCoordsProj.z >= -1.0 + SHADOW_CASCADE_SEAM_INSET && shadowTexCoordsProj.z < 1.0 - SHADOW_CASCADE_SEAM_INSET)
        {
            ...
            shadowScalar = pow(shadowScalar, commonData.lightShadowDensity * densityScalar);
            return shadowScalar;
        }
    }
    return 1.0;
...
vec3 computeFogAccumPoint(vec4 position, int lightIndex)
{
    ...
    // compute ray info
    vec3 startPosition = commonData.eyeCenter;
    ...
    // compute step info
    float stepLength = rayLength / commonData.ssvfSteps;
    ...
    if (shadowIndex < 0)
    {
        ...
        for (int i = 0; i < commonData.ssvfSteps; ++i)
        {
            // compute intensity inside light volume
            vec3 v = normalize(commonData.eyeCenter - currentPosition);
            vec3 d = lightOrigin - currentPosition;
            vec3 l = normalize(d);
            vec3 h = normalize(v + l);
            float distanceSquared = dot(d, d);
            float distance = sqrt(distanceSquared);
            float cutoffScalar = 1.0 - smoothstep(lightCutoff * (1.0 - commonData.lightCutoffMargin), lightCutoff, distance);
            ...
            float asymmetrySquared = commonData.ssvfAsymmetry * commonData.ssvfAsymmetry;
            float fogMoment = (1.0 - asymmetrySquared) / (4.0 * PI * pow(1.0 + asymmetrySquared - 2.0 * commonData.ssvfAsymmetry * theta, 1.5));
    ...
    // fin
    return smoothstep(0.0, 1.0, result / validSteps) * light.lightColors * light.lightBrightnesses * commonData.ssvfIntensity;
}
...
vec3 computeFogAccumDirectional(vec4 position, int lightIndex)
{
    ...
    // fin
    return smoothstep(0.0, 1.0, result / commonData.ssvfSteps) * light.lightColors * light.lightBrightnesses * commonData.ssvfIntensity;
}
...
vec3 computeFogAccumCascaded(vec4 position, int lightIndex)
{
    ...
    // fin
    return smoothstep(0.0, 1.0, result / (commonData.ssvfSteps * SHADOW_CASCADE_LEVELS)) * light.lightColors * light.lightBrightnesses * commonData.ssvfIntensity;
}
...
void computeSsrr(float depth, vec4 position, vec3 normal, float refractiveIndex, float subsurfaceCutoff, float subsurfaceCutoffMargin, inout vec3 diffuseScreen, inout float diffuseSurfaceWeight, inout float diffuseScreenWeight)
{
    ...
    vec4 stopView = vec4(positionView.xyz + refractionView * commonData.ssrrDistanceCutoff, 1.0);
    ...
    float stepCount = abs(shouldMarchHorizontal ? marchHorizontal : marchVertical) * commonData.ssrrDetail;
    ...
    for (int i = 0; i < stepCount && currentTexCoords.x >= 0.0 && currentTexCoords.x < 1.0 && currentTexCoords.y >= 0.0 && currentTexCoords.y < 1.0; ++i)
    {
        ...
        float thickness = max(pow(-currentPositionView.z, 32.0) * commonData.ssrrRayThickness, commonData.ssrrRayThickness);
        ...
        if (currentDepth != 0.0 && depthDelta >= 0.0 && depthDelta <= thickness)
        {
            ...
            for (int j = 0; j < commonData.ssrrRefinementsMax; ++j)
            {
                // advance frag values
                ...
                if (currentDepth != 0.0 && depthDelta >= 0.0 && depthDelta <= thickness)
                {
                    ...
                    diffuseScreen = texture(sampler2D(colorTexture, colorSampler), currentTexCoords).rgb * commonData.ssrrIntensity;
                    ...
                    diffuseScreenWeight =
                        (1.0 - smoothstep(1.0 - commonData.ssrrDistanceCutoffMargin, 1.0, length(currentPositionView - positionView) / commonData.ssrrDistanceCutoff)) * // filter out as reflection point reaches max distance from fragment
                        smoothstep(0.0, 0.5, eyeDistanceFromPlane) * // filter out as eye nears plane
                        smoothstep(0.0, commonData.ssrrEdgeHorizontalMargin, min(currentTexCoords.x, 1.0 - currentTexCoords.x)) *
                        smoothstep(0.0, commonData.ssrrEdgeVerticalMargin, min(currentTexCoords.y, 1.0 - currentTexCoords.y));
...
void main()
{
    ...
    float distance = length(position.xyz - commonData.eyeCenter);
    ...
    vec3 v = normalize(commonData.eyeCenter - position.xyz);
    ...
    for (int i = 0; i < lightsGeneral.lightsCount; ++i)
    {
        ...
        // accumulate fog
        if (commonData.ssvfEnabled == 1 && light.lightDesireFogs == 1)
        {
            switch (lightType)
            {
    ...
    bool ssrrDesired = commonData.ssrrEnabled == 1 && refractiveIndex != 1.0;
    vec3 environmentFilterRefracted = vec3(0.0);
    if (lm1 == -1 && lm2 == -1)
    {
        ambientColor = commonData.lightAmbientColor;
        ambientBrightness = commonData.lightAmbientBrightness;
    ...
    else if (lm2 == -1)
    {
        ...
        vec3 ambientColor2 = commonData.lightAmbientColor;
        float ambientBrightness1 = lightMap1.lightMapAmbientBrightnesses;
        float ambientBrightness2 = commonData.lightAmbientBrightness;
    ...
    float ambientBoostFactor = smoothstep(1.0 - commonData.lightAmbientBoostCutoff, 1.0, 1.0 - roughness);
    float ambientBoost = 1.0 + ambientBoostFactor * commonData.lightAmbientBoostScalar;
    ...
    vec3 ambientColorRefracted = environmentFilterRefractedSaturated * ambientBrightness * commonData.ssrrIntensity;
    ...
    if (commonData.fogEnabled == 1)
    {
        switch (commonData.fogType)
        {
            case 0: // linear
            {
                float fogFactor = smoothstep(commonData.fogStart / commonData.fogFinish, 1.0, min(1.0, distance / commonData.fogFinish)) * commonData.fogColor.a;
                color = color * (1.0 - fogFactor) + commonData.fogColor.rgb * fogFactor;
                break;
            }
            case 1: // exponential
            {
                float fogFactor = (1.0 - exp(-commonData.fogDensity * distance)) * commonData.fogColor.a;
                color = color * (1.0 - fogFactor) + commonData.fogColor.rgb * fogFactor;
                break;
            }
            default: // exponential squared
            {
                float fogFactor = (1.0 - exp(-commonData.fogDensity * commonData.fogDensity * distance * distance)) * commonData.fogColor.a;
                color = color * (1.0 - fogFactor) + commonData.fogColor.rgb * fogFactor;
                break;
            }
        }
