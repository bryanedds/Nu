...
layout(binding = 35) uniform sampler2D albedoTexture;
layout(binding = 36) uniform sampler2D roughnessTexture;
layout(binding = 37) uniform sampler2D metallicTexture;
layout(binding = 38) uniform sampler2D ambientOcclusionTexture;
layout(binding = 39) uniform sampler2D emissionTexture;
layout(binding = 40) uniform sampler2D normalTexture;
layout(binding = 41) uniform sampler2D heightTexture;
...
layout(binding = 47) uniform samplerCube irradianceMaps[LIGHT_MAPS_MAX];
layout(binding = 48) uniform samplerCube environmentFilterMaps[LIGHT_MAPS_MAX];
layout(binding = 49) uniform sampler2DArray shadowTextures;
layout(binding = 50) uniform samplerCube shadowMaps[SHADOW_MAPS_MAX];
layout(binding = 51) uniform sampler2DArray shadowCascades[SHADOW_CASCADES_MAX];
layout(binding = 52) uniform i1 { vec3 lightMapOrigins[LIGHT_MAPS_MAX]; } lightMapOrigins;
layout(binding = 53) uniform j1 { vec3 lightMapMins[LIGHT_MAPS_MAX]; } lightMapMins;
layout(binding = 54) uniform k1 { vec3 lightMapSizes[LIGHT_MAPS_MAX]; } lightMapSizes;
layout(binding = 55) uniform l1 { vec3 lightMapAmbientColors[LIGHT_MAPS_MAX]; } lightMapAmbientColors;
layout(binding = 56) uniform m1 { float lightMapAmbientBrightnesses[LIGHT_MAPS_MAX]; } lightMapAmbientBrightnesses;
layout(binding = 57) uniform n1 { int lightMapsCount; } lightMapsCount;
layout(binding = 58) uniform o1 { float lightMapSingletonBlendMargin; } lightMapSingletonBlendMargin;
layout(binding = 59) uniform p1 { vec3 lightOrigins[LIGHTS_MAX]; } lightOrigins;
layout(binding = 60) uniform q1 { vec3 lightDirections[LIGHTS_MAX]; } lightDirections;
layout(binding = 61) uniform r1 { vec3 lightColors[LIGHTS_MAX]; } lightColors;
layout(binding = 62) uniform s1 { float lightBrightnesses[LIGHTS_MAX]; } lightBrightnesses;
layout(binding = 63) uniform t1 { float lightAttenuationLinears[LIGHTS_MAX]; } lightAttenuationLinears;
layout(binding = 64) uniform u1 { float lightAttenuationQuadratics[LIGHTS_MAX]; } lightAttenuationQuadratics;
layout(binding = 65) uniform v1 { float lightCutoffs[LIGHTS_MAX]; } lightCutoffs;
layout(binding = 66) uniform w1 { int lightTypes[LIGHTS_MAX]; } lightTypes;
layout(binding = 67) uniform x1 { float lightConeInners[LIGHTS_MAX]; } lightConeInners;
layout(binding = 68) uniform y1 { float lightConeOuters[LIGHTS_MAX]; } lightConeOuters;
layout(binding = 69) uniform z1 { int lightDesireFogs[LIGHTS_MAX]; } lightDesireFogs;
layout(binding = 70) uniform a2 { int lightShadowIndices[LIGHTS_MAX]; } lightShadowIndices;
layout(binding = 71) uniform b2 { int lightsCount; } lightsCount;
layout(binding = 72) uniform c2 { float shadowNear; } shadowNear; // NOTE: currently unused.
layout(binding = 73) uniform d2 { mat4 shadowMatrices[SHADOW_TEXTURES_MAX + SHADOW_CASCADES_MAX * SHADOW_CASCADE_LEVELS]; } shadowMatrices;
