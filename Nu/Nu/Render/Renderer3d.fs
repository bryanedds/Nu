...
type [<ReferenceEquality>] private RenderTasks =
    { SkyBoxes : (Color * single * Color * single * CubeMap AssetTag) List
      ...
      mutable ShadowValidated : int option }
    ...
    static member clear renderTasks =
        ...
        renderTasks.ShadowValidated <- None
...
type [<ReferenceEquality>] GlRenderer3d =
    ...
    static member private renderGeometry
        ...
        OpenGL.PhysicallyBased.DrawPhysicallyBasedDeferredLightingSurface
            (eyeCenter, viewArray, rasterProjectionArray, renderer.LightingConfig.LightCutoffMargin, renderer.LightingConfig.LightShadowSampleScalar, renderer.LightingConfig.LightShadowExponent, renderer.LightingConfig.LightShadowDensity,
    ...
    static member render frustumInterior frustumExterior frustumImposter lightBox eyeCenter (eyeRotation : Quaternion) windowSize renderbuffer framebuffer renderMessages renderer =
                    ...
                    if renderTasks.Populated then
                        ...
                        | ShadowPass (shadowLightId, shadowLightType, shadowRotation, _) when lightId = shadowLightId && shadowTextureBufferIndex < Constants.Render.ShadowTexturesMax ->
                            ...
                            let draw =
                                ...
                                | (true, renderTasksCached) when renderTasksCached.ShadowValidated = Some shadowTextureBufferIndex -> not (RenderTasks.shadowUpToDate renderTasks renderTasksCached)
                                | (_, _) -> true
                            if draw then
                            ...
                            // validate shadow
                            renderTasks.ShadowValidated <- Some shadowTextureBufferIndex
                    ...
                    if renderTasks.Populated then
                        ...
                        | ShadowPass (shadowLightId, shadowLightType, _, _) when lightId = shadowLightId && shadowMapBufferIndex < Constants.Render.ShadowMapsMax ->
                            ...
                            | PointLight ->
                                ...
                                let draw =
                                    match renderer.RenderPasses2.TryGetValue renderPass with
                                    | (true, renderTasksCached) when renderTasksCached.ShadowValidated = Some (shadowMapBufferIndex + Constants.Render.ShadowTexturesMaxShader) -> not (RenderTasks.shadowUpToDate renderTasks renderTasksCached)
                                    | (_, _) -> true
                                if draw then
                                ...
                                // validate shadow
                                renderTasks.ShadowValidated <- Some (shadowMapBufferIndex + Constants.Render.ShadowTexturesMaxShader)
        ...
        // swap render passes
        for renderTasks in renderer.RenderPasses.Values do if renderTasks.ShadowValidated.IsNone then RenderTasks.clear renderTasks
