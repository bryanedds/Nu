...
type TerrainPatch =
    { PatchBounds : Box3
      ...
      VertexOffset : Vector2i // Starting vertex offset for this patch in the heightmap
      VertexSize : Vector2i } // Size of the patch in vertices
...
type TerrainGeometryDescriptor =
    { Bounds : Box3
      ...
      HeightMap : HeightMap
      Segments : Vector2i
      PatchOpt : TerrainPatch option } // None for single-mesh terrain, Some for patch-based terrain
...
type TerrainDescriptor =
    { Bounds : Box3
      ...
      HeightMap : HeightMap
      Segments : Vector2i
      PatchOpt : TerrainPatch option } // None for legacy single-mesh terrain, Some for patch-based terrain
...
module TerrainPatch =
    ...
    let calculateTerrainPatches (bounds : Box3) (resolution : Vector2i) =
        ...
        let patchCountY = (resolution.Y + patchSize - 1) / patchSize
        
        let patchSizeX = bounds.Size.X / single patchCountX
        let patchSizeY = bounds.Size.Z / single patchCountY
        
        [| for py in 0 .. patchCountY - 1 do
            for px in 0 .. patchCountX - 1 do
                let vertexOffsetX = px * patchSize
                let vertexOffsetY = py * patchSize
                let vertexSizeX = min patchSize (resolution.X - vertexOffsetX)
                let vertexSizeY = min patchSize (resolution.Y - vertexOffsetY)
                
                let patchMinX = bounds.Min.X + single px * patchSizeX
                let patchMinZ = bounds.Min.Z + single py * patchSizeY
                let patchMaxX = bounds.Min.X + single (px + 1) * patchSizeX
                let patchMaxZ = bounds.Min.Z + single (py + 1) * patchSizeY
                
                { PatchBounds = box3 (v3 patchMinX bounds.Min.Y patchMinZ) (v3 patchMaxX bounds.Max.Y patchMaxZ)
                  PatchIndices = v2i px py
                  VertexOffset = v2i vertexOffsetX vertexOffsetY
                  VertexSize = v2i vertexSizeX vertexSizeY } |]
...
type [<ReferenceEquality>] GlRenderer3d =
        ...
        | ValueSome heightMapMetadata ->
            ...
            | (Some normals, Some tint) ->
                ...
                | Some patch ->
                    ...
                    let patchVertices =
                        [|for y in patch.VertexOffset.Y .. patch.VertexOffset.Y + patch.VertexSize.Y - 1 do
                            for x in patch.VertexOffset.X .. patch.VertexOffset.X + patch.VertexSize.X - 1 do
                                if y < resolution.Y && x < resolution.X then
                                    let i = y * resolution.X + x
                    ...
                    // compute patch indices with adjusted coordinates
                    let patchIndices = 
                        [|for y in 0 .. patch.VertexSize.Y - 2 do
                            for x in 0 .. patch.VertexSize.X - 2 do
                                let baseIndex = y * patch.VertexSize.X + x
                                yield baseIndex
                                yield baseIndex + patch.VertexSize.X
                                yield baseIndex + 1
                                yield baseIndex + patch.VertexSize.X
                                yield baseIndex + patch.VertexSize.X + 1
                                yield baseIndex + 1|]
    ...
    static member private categorizeTerrain
        ...
        // determine if we should use patches based on terrain size
        let shouldUsePatchesOpt =
            match terrainDescriptor.HeightMap with
            | ImageHeightMap _ | RawHeightMap _ ->
                let heightMapMetadataOpt = 
                    HeightMap.tryGetMetadata
                        (fun assetTag -> GlRenderer3d.tryGetFilePath assetTag renderer)
                ...
                | ValueSome heightMapMetadata ->
                    let resolution = heightMapMetadata.Resolution
                    // Use patches if terrain is larger than patch size in either dimension
                    if resolution.X > Constants.Render.TerrainPatchSize || resolution.Y > Constants.Render.TerrainPatchSize
                    then Some (TerrainPatch.calculateTerrainPatches terrainDescriptor.Bounds resolution)
                    else None
                | ValueNone -> None

        match shouldUsePatchesOpt with
        | Some patches ->
            // render terrain as patches for better GPU performance
            for patch in patches do
                let patchGeometryDescriptor = 
                    { terrainDescriptor.TerrainGeometryDescriptor with PatchOpt = Some patch }
                
                // attempt to create patch geometry
                match renderer.PhysicallyBasedTerrainGeometries.TryGetValue patchGeometryDescriptor with
                | (true, _) -> ()
                | (false, _) ->
                    match GlRenderer3d.tryCreatePhysicallyBasedTerrainGeometry patchGeometryDescriptor renderer with
        ...
        | None ->
            // render as single terrain (legacy behavior)
    ...
    static member private renderGeometry
        ...
        OpenGL.PhysicallyBased.DrawPhysicallyBasedDeferredLightingSurface
            (eyeCenter, viewArray, viewInverseArray, rasterProjectionArray, rasterProjectionInverseArray, renderer.LightingConfig.LightCutoffMargin,
             ...
             depthTexture, albedoTexture, materialTexture, normalPlusTexture, subdermalPlusTexture, scatterPlusTexture, shadowTextures, shadowMaps,
        ...
        let (colorTexture, depthTexture2, lightingRenderbuffer, lightingFramebuffer) = renderer.PhysicallyBasedBuffers.ColoringBuffers
        OpenGL.Gl.BindRenderbuffer (OpenGL.RenderbufferTarget.Renderbuffer, lightingRenderbuffer)
        OpenGL.Gl.BindFramebuffer (OpenGL.FramebufferTarget.Framebuffer, lightingFramebuffer)
        OpenGL.Gl.ClearColor (Constants.Render.ViewportClearColor.R, Constants.Render.ViewportClearColor.G, Constants.Render.ViewportClearColor.B, Constants.Render.ViewportClearColor.A)
        OpenGL.Gl.Clear (OpenGL.ClearBufferMask.ColorBufferBit ||| OpenGL.ClearBufferMask.DepthBufferBit ||| OpenGL.ClearBufferMask.StencilBufferBit)
        ...
        let fogAccumTexture =
            ...
            if renderer.LightingConfig.SsvfEnabled then
                ...
                // efficiency.
                OpenGL.PhysicallyBased.DrawFilterBilateralDownSampleSurface (fogAccumTexture, depthTexture2, renderer.PhysicallyBasedQuad, renderer.FilterBilateralDownSample4dShader, renderer.PhysicallyBasedStaticVao)
                ...
                OpenGL.PhysicallyBased.DrawFilterBilateralUpSampleSurface (fogAccumDownSampleTexture, depthDownSampleTexture, depthTexture2, renderer.PhysicallyBasedQuad, renderer.FilterBilateralUpSample4dShader, renderer.PhysicallyBasedStaticVao)
        ...
        OpenGL.PhysicallyBased.DrawPhysicallyBasedDeferredCompositionSurface
            (eyeCenter, viewInverseArray, rasterProjectionInverseArray, fogEnabled, renderer.LightingConfig.FogStart, renderer.LightingConfig.FogFinish, renderer.LightingConfig.FogColor,
             depthTexture, colorTexture, fogAccumTexture, renderer.PhysicallyBasedQuad, renderer.PhysicallyBasedShaders.DeferredCompositionShader, renderer.PhysicallyBasedStaticVao)
        ...
        for (shader, vao) in forwardShaderAndVaos do
            GlRenderer3d.beginPhysicallyBasedForwardShader
                viewArray rasterProjectionArray rasterViewProjectionArray eyeCenter renderer.LightingConfig.LightCutoffMargin lightAmbientColor lightAmbientBrightness renderer.LightingConfig.LightAmbientBoostCutoff renderer.LightingConfig.LightAmbientBoostScalar
                renderer.LightingConfig.LightShadowSamples renderer.LightingConfig.LightShadowBias renderer.LightingConfig.LightShadowSampleScalar renderer.LightingConfig.LightShadowExponent renderer.LightingConfig.LightShadowDensity
    ...
    static member render
        ...
        for struct (lightId, lightOrigin, lightCutoff, lightConeOuter, lightDesireShadows, lightBounds) in spotAndDirectionalLightsArray do
            if lightDesireShadows = 1 && lightBox.Intersects lightBounds then
                for (renderPass, renderTasks) in renderer.RenderPasses.Pairs do
                    match renderPass with
                    | ShadowPass (shadowLightId, shadowFaceInfoOpt, shadowLightType, shadowRotation, shadowFrustum) when lightId = shadowLightId && shadowFaceInfoOpt.IsNone && shadowTextureBufferIndex < Constants.Render.ShadowTexturesMax ->
        ...
        for struct (lightId, lightOrigin, lightCutoff, _, lightDesireShadows, lightBounds) in pointLightsArray do
            if lightDesireShadows = 1 && lightBox.Intersects lightBounds then
                for (renderPass, renderTasks) in renderer.RenderPasses.Pairs do
                    match renderPass with
                    | ShadowPass (shadowLightId, shadowFaceInfoOpt, shadowLightType, _, shadowFrustum) when lightId = shadowLightId && shadowFaceInfoOpt.IsSome && shadowMapBufferIndex < Constants.Render.ShadowMapsMax ->
                        match shadowLightType with
                        | PointLight ->
    ...
    static member make glContext window geometryViewport rasterViewport =
        ...
        // create filter gaussian 4d shader
