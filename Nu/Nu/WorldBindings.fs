...
module WorldBindings =
    ...
    let initBindings () =
        let bindings =
            [
             ("resolve", { Fn = evalResolveBinding; Pars = []; DocOpt = None })
             ...
             ("selectScreen", { Fn = evalSelectScreenBinding; Pars = []; DocOpt = None })
             ("tryTransitionScreen", { Fn = evalTryTransitionScreenBinding; Pars = []; DocOpt = None })
             ("transitionScreen", { Fn = evalTransitionScreenBinding; Pars = []; DocOpt = None })
             ("createDissolveScreenFromLayerFile6", { Fn = evalCreateDissolveScreenFromLayerFile6Binding; Pars = []; DocOpt = None })
             ("createDissolveScreenFromLayerFile", { Fn = evalCreateDissolveScreenFromLayerFileBinding; Pars = []; DocOpt = None })
             ("createSplashScreen6", { Fn = evalCreateSplashScreen6Binding; Pars = []; DocOpt = None })
             ("createSplashScreen", { Fn = evalCreateSplashScreenBinding; Pars = []; DocOpt = None })
             ("getEntitiesInView2", { Fn = evalGetEntitiesInView2Binding; Pars = []; DocOpt = None })
             ("getEntitiesInBounds3", { Fn = evalGetEntitiesInBounds3Binding; Pars = []; DocOpt = None })
             ("getEntitiesAtPoint3", { Fn = evalGetEntitiesAtPoint3Binding; Pars = []; DocOpt = None })
             ("getEntitiesInView", { Fn = evalGetEntitiesInViewBinding; Pars = []; DocOpt = None })
             ("getEntitiesInBounds", { Fn = evalGetEntitiesInBoundsBinding; Pars = []; DocOpt = None })
             ("getEntitiesAtPoint", { Fn = evalGetEntitiesAtPointBinding; Pars = []; DocOpt = None })
             ("playSong", { Fn = evalPlaySongBinding; Pars = []; DocOpt = None })
             ("playSong4", { Fn = evalPlaySong4Binding; Pars = []; DocOpt = None })
             ("playSound", { Fn = evalPlaySoundBinding; Pars = []; DocOpt = None })
             ("playSound3", { Fn = evalPlaySound3Binding; Pars = []; DocOpt = None })
             ("fadeOutSong", { Fn = evalFadeOutSongBinding; Pars = []; DocOpt = None })
             ("stopSong", { Fn = evalStopSongBinding; Pars = []; DocOpt = None })
             ("hintAudioPackageUse", { Fn = evalHintAudioPackageUseBinding; Pars = []; DocOpt = None })
             ("hintAudioPackageDisuse", { Fn = evalHintAudioPackageDisuseBinding; Pars = []; DocOpt = None })
             ("reloadAudioAssets", { Fn = evalReloadAudioAssetsBinding; Pars = []; DocOpt = None })
             ("hintRenderPackageUse", { Fn = evalHintRenderPackageUseBinding; Pars = []; DocOpt = None })
             ("hintRenderPackageDisuse", { Fn = evalHintRenderPackageDisuseBinding; Pars = []; DocOpt = None })
             ("reloadRenderAssets", { Fn = evalReloadRenderAssetsBinding; Pars = []; DocOpt = None })
             ("bodyExists", { Fn = evalBodyExistsBinding; Pars = []; DocOpt = None })
             ("getBodyContactNormals", { Fn = evalGetBodyContactNormalsBinding; Pars = []; DocOpt = None })
             ("getBodyLinearVelocity", { Fn = evalGetBodyLinearVelocityBinding; Pars = []; DocOpt = None })
             ("getBodyToGroundContactNormals", { Fn = evalGetBodyToGroundContactNormalsBinding; Pars = []; DocOpt = None })
             ("getBodyToGroundContactNormalOpt", { Fn = evalGetBodyToGroundContactNormalOptBinding; Pars = []; DocOpt = None })
             ("getBodyToGroundContactTangentOpt", { Fn = evalGetBodyToGroundContactTangentOptBinding; Pars = []; DocOpt = None })
             ("isBodyOnGround", { Fn = evalIsBodyOnGroundBinding; Pars = []; DocOpt = None })
             ("createBody", { Fn = evalCreateBodyBinding; Pars = []; DocOpt = None })
             ("createBodies", { Fn = evalCreateBodiesBinding; Pars = []; DocOpt = None })
             ("destroyBody", { Fn = evalDestroyBodyBinding; Pars = []; DocOpt = None })
             ("destroyBodies", { Fn = evalDestroyBodiesBinding; Pars = []; DocOpt = None })
             ("setBodyPosition", { Fn = evalSetBodyPositionBinding; Pars = []; DocOpt = None })
             ("setBodyRotation", { Fn = evalSetBodyRotationBinding; Pars = []; DocOpt = None })
             ("setBodyAngularVelocity", { Fn = evalSetBodyAngularVelocityBinding; Pars = []; DocOpt = None })
             ("setBodyLinearVelocity", { Fn = evalSetBodyLinearVelocityBinding; Pars = []; DocOpt = None })
             ("applyBodyAngularImpulse", { Fn = evalApplyBodyAngularImpulseBinding; Pars = []; DocOpt = None })
             ("applyBodyLinearImpulse", { Fn = evalApplyBodyLinearImpulseBinding; Pars = []; DocOpt = None })
             ("applyBodyForce", { Fn = evalApplyBodyForceBinding; Pars = []; DocOpt = None })
             ("isMouseButtonDown", { Fn = evalIsMouseButtonDownBinding; Pars = []; DocOpt = None })
             ...
             ("isKeyboardKeyDown", { Fn = evalIsKeyboardKeyDownBinding; Pars = []; DocOpt = None })
             ("getSimulantSelected", { Fn = evalGetSimulantSelectedBinding; Pars = []; DocOpt = None })
             ("tryGetSimulantParent", { Fn = evalTryGetSimulantParentBinding; Pars = []; DocOpt = None })
             ("getSimulantChildren", { Fn = evalGetSimulantChildrenBinding; Pars = []; DocOpt = None })
             ("getSimulantExists", { Fn = evalGetSimulantExistsBinding; Pars = []; DocOpt = None })
             ("getEntities0", { Fn = evalGetEntities0Binding; Pars = []; DocOpt = None })
             ("getLayers0", { Fn = evalGetLayers0Binding; Pars = []; DocOpt = None })
             ("isSimulantSelected", { Fn = evalIsSimulantSelectedBinding; Pars = []; DocOpt = None })
             ("writeGameToFile", { Fn = evalWriteGameToFileBinding; Pars = []; DocOpt = None })
             ("readGameFromFile", { Fn = evalReadGameFromFileBinding; Pars = []; DocOpt = None })
             ("getScreens", { Fn = evalGetScreensBinding; Pars = []; DocOpt = None })
             ("destroyScreen", { Fn = evalDestroyScreenBinding; Pars = []; DocOpt = None })
             ("createScreen", { Fn = evalCreateScreenBinding; Pars = []; DocOpt = None })
             ("createDissolveScreen", { Fn = evalCreateDissolveScreenBinding; Pars = []; DocOpt = None })
             ("writeScreenToFile", { Fn = evalWriteScreenToFileBinding; Pars = []; DocOpt = None })
             ("readScreenFromFile", { Fn = evalReadScreenFromFileBinding; Pars = []; DocOpt = None })
             ("getLayers", { Fn = evalGetLayersBinding; Pars = []; DocOpt = None })
             ("destroyLayer", { Fn = evalDestroyLayerBinding; Pars = []; DocOpt = None })
             ("destroyLayers", { Fn = evalDestroyLayersBinding; Pars = []; DocOpt = None })
             ("writeLayerToFile", { Fn = evalWriteLayerToFileBinding; Pars = []; DocOpt = None })
             ("readLayerFromFile", { Fn = evalReadLayerFromFileBinding; Pars = []; DocOpt = None })
             ("getEntities", { Fn = evalGetEntitiesBinding; Pars = []; DocOpt = None })
             ("destroyEntity", { Fn = evalDestroyEntityBinding; Pars = []; DocOpt = None })
             ("destroyEntities", { Fn = evalDestroyEntitiesBinding; Pars = []; DocOpt = None })
             ("tryPickEntity", { Fn = evalTryPickEntityBinding; Pars = []; DocOpt = None })
             ("createEntity", { Fn = evalCreateEntityBinding; Pars = []; DocOpt = None })
             ("reassignEntity", { Fn = evalReassignEntityBinding; Pars = []; DocOpt = None })
             ("trySetEntityOverlayNameOpt", { Fn = evalTrySetEntityOverlayNameOptBinding; Pars = []; DocOpt = None })
             ("trySetEntityFacetNames", { Fn = evalTrySetEntityFacetNamesBinding; Pars = []; DocOpt = None })
             ("createLayer", { Fn = evalCreateLayerBinding; Pars = []; DocOpt = None })
             ("getEyeCenter", { Fn = evalGetEyeCenterBinding; Pars = []; DocOpt = None })
             ("setEyeCenter", { Fn = evalSetEyeCenterBinding; Pars = []; DocOpt = None })
             ("getEyeSize", { Fn = evalGetEyeSizeBinding; Pars = []; DocOpt = None })
             ("setEyeSize", { Fn = evalSetEyeSizeBinding; Pars = []; DocOpt = None })
             ("getOmniscreenOpt", { Fn = evalGetOmniscreenOptBinding; Pars = []; DocOpt = None })
             ("setOmniscreenOpt", { Fn = evalSetOmniscreenOptBinding; Pars = []; DocOpt = None })
             ("getOmniscreen", { Fn = evalGetOmniscreenBinding; Pars = []; DocOpt = None })
             ("setOmniscreen", { Fn = evalSetOmniscreenBinding; Pars = []; DocOpt = None })
             ("getSelectedScreenOpt", { Fn = evalGetSelectedScreenOptBinding; Pars = []; DocOpt = None })
             ("setSelectedScreenOpt", { Fn = evalSetSelectedScreenOptBinding; Pars = []; DocOpt = None })
             ("getSelectedScreen", { Fn = evalGetSelectedScreenBinding; Pars = []; DocOpt = None })
             ("setSelectedScreen", { Fn = evalSetSelectedScreenBinding; Pars = []; DocOpt = None })
             ...
             ("getViewBounds", { Fn = evalGetViewBoundsBinding; Pars = []; DocOpt = None })
             ("isBoundsInView", { Fn = evalIsBoundsInViewBinding; Pars = []; DocOpt = None })
             ("mouseToScreen", { Fn = evalMouseToScreenBinding; Pars = []; DocOpt = None })
             ("mouseToWorld", { Fn = evalMouseToWorldBinding; Pars = []; DocOpt = None })
             ("mouseToEntity", { Fn = evalMouseToEntityBinding; Pars = []; DocOpt = None })
             ("getTickRate", { Fn = evalGetTickRateBinding; Pars = []; DocOpt = None })
             ("getTickRateF", { Fn = evalGetTickRateFBinding; Pars = []; DocOpt = None })
             ("setTickRate", { Fn = evalSetTickRateBinding; Pars = []; DocOpt = None })
             ...
             ("tryGetTextureSize", { Fn = evalTryGetTextureSizeBinding; Pars = []; DocOpt = None })
             ("getTextureSize", { Fn = evalGetTextureSizeBinding; Pars = []; DocOpt = None })
             ("tryGetTextureSizeAsVector2", { Fn = evalTryGetTextureSizeAsVector2Binding; Pars = []; DocOpt = None })
             ("getTextureSizeAsVector2", { Fn = evalGetTextureSizeAsVector2Binding; Pars = []; DocOpt = None })
            ] |>
            dictPlus
