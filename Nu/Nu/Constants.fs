...
module Engine =
    ...
    let (*Literal*) QuadtreeBounds = Box2 (-QuadtreeSize * 0.5f, QuadtreeSize * 0.5f)
    let (*Literal*) OctreeGranularity = 4
    let (*Literal*) OctreeDepth = 3
    let (*Literal*) OctreeSize = Vector3 (single (Math.Pow (2.0, 16.0)))
    let (*Literal*) OctreeBounds = Box3 (-OctreeSize * 0.5f, OctreeSize * 0.5f)
