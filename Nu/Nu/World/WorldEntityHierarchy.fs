            ...
            | ValueSome staticModelMetadata ->
                ...
                staticModelMetadata.PhysicallyBasedHierarchy.Traverse (fun nodes ->
                    for node in nodes do
                        match node with
                        | Vulkan.PhysicallyBasedNode names ->
                        ...
                        | Vulkan.PhysicallyBasedLightProbe lightProbe ->
                            let (mountToParent, surnames, group) =
                                match parent with
                            ...
                            child.SetStatic true world
                            child.AutoBounds world
                        | Vulkan.PhysicallyBasedLight light ->
                            let (mountToParent, surnames, group) =
                                match parent with
                            ...
                            child.SetStatic true world
                            child.AutoBounds world
                        | Vulkan.PhysicallyBasedSurface surface ->
                            let (mountToParent, surnames, group) =
                                match parent with
                            ...
                            let child =
                                if rigid then
                                    ...
                                    let navShape = Vulkan.PhysicallyBasedSurfaceFns.extractNavShape ContourNavShape staticModelMetadata.SceneOpt surface
                            ...
                            let presence = Vulkan.PhysicallyBasedSurfaceFns.extractPresence presenceConferred staticModelMetadata.SceneOpt surface
                            let renderStyle = Vulkan.PhysicallyBasedSurfaceFns.extractRenderStyle Deferred staticModelMetadata.SceneOpt surface
                            let ignoreLightMaps = Vulkan.PhysicallyBasedSurfaceFns.extractIgnoreLightMaps Constants.Render.IgnoreLightMapsDefault staticModelMetadata.SceneOpt surface
                            let opaqueDistance = Vulkan.PhysicallyBasedSurfaceFns.extractOpaqueDistance Constants.Render.OpaqueDistanceDefault staticModelMetadata.SceneOpt surface
                            let finenessOffset = Vulkan.PhysicallyBasedSurfaceFns.extractFinenessOffset Constants.Render.FinenessOffsetDefault staticModelMetadata.SceneOpt surface
                            let scatterType = Vulkan.PhysicallyBasedSurfaceFns.extractScatterType Constants.Render.ScatterTypeDefault staticModelMetadata.SceneOpt surface
                            let specularScalar = Vulkan.PhysicallyBasedSurfaceFns.extractSpecularScalar Constants.Render.SpecularScalarDefault staticModelMetadata.SceneOpt surface
                            let subsurfaceCutoff = Vulkan.PhysicallyBasedSurfaceFns.extractSubsurfaceCutoff Constants.Render.SubsurfaceCutoffDefault staticModelMetadata.SceneOpt surface
                            let subsurfaceCutoffMargin = Vulkan.PhysicallyBasedSurfaceFns.extractSubsurfaceCutoffMargin Constants.Render.SubsurfaceCutoffMarginDefault staticModelMetadata.SceneOpt surface
                    ...
                    if entity.GetSurfaceFreezable world then // NOTE: shouldn't matter in practice, but there are O(n^2) calls to GetFreezable implicated here.
                        if getType (entity.GetDispatcher world) = typeof<Entity3dDispatcher> then
                            ...
                            while surfaceIndex < metadata.Surfaces.Length do
                                ...
                                let presence = Vulkan.PhysicallyBasedSurfaceFns.extractPresence transform.Presence metadata.SceneOpt surface
                                let renderStyle = Vulkan.PhysicallyBasedSurfaceFns.extractRenderStyle (entity.GetRenderStyle world) metadata.SceneOpt surface
                                let renderType = match renderStyle with Deferred -> DeferredRenderType | Forward (subsort, sort) -> ForwardRenderType (subsort, sort)
                                let ignoreLightMaps = Vulkan.PhysicallyBasedSurfaceFns.extractIgnoreLightMaps properties.IgnoreLightMaps metadata.SceneOpt surface
                                ...
                                let finenessOffset = Vulkan.PhysicallyBasedSurfaceFns.extractFinenessOffset properties.FinenessOffset metadata.SceneOpt surface
                                ...
                                let scatterType = Vulkan.PhysicallyBasedSurfaceFns.extractScatterType properties.ScatterType metadata.SceneOpt surface
