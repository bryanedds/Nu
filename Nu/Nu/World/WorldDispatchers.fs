                ...
                staticModelMetadata.PhysicallyBasedHierarchy.Traverse (fun nodes ->
                    for node in nodes do
                        ...
                        | Left names ->
                            ...
                            world <- world' // using mutable world here to interface w/ imperative API
                            world <- child.SetPersistent false world
                            world <- child.SetStatic (entity.GetStatic world) world
                            world <- child.SetMountOpt (Some (Relation.makeParent ())) world
                        | Right surface ->
                            ...
                            world <- child.SetSurfaceIndex i world
                            world <- child.SetStaticModel staticModel world
                            world <- child.SetAlbedoOpt (Some surface.PhysicallyBasedMaterial.Albedo) world
                            world <- child.SetMetalnessOpt (Some surface.PhysicallyBasedMaterial.Metalness) world
                            world <- child.SetRoughnessOpt (Some surface.PhysicallyBasedMaterial.Roughness) world
                            world <- child.SetAmbientOcclusionOpt (Some surface.PhysicallyBasedMaterial.AmbientOcclusion) world
                            world <- child.SetPositionLocal position world
                            world <- child.SetRotationLocal rotation world
                            world <- child.SetScaleLocal scale world
                            world <- child.SetSize boundsExtended.Size world
                            world <- child.SetPersistent false world
                            world <- child.SetStatic (entity.GetStatic world) world
                            world <- child.SetMountOpt (Some (Relation.makeParent ())) world
                            i <- inc i)
