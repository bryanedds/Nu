...
type Contour2dDispatcher (physical, lightProbe, light) =
    inherit EntityDispatcher (true, Constants.Engine.Entity2dPerimeterCenteredDefault, physical, lightProbe, light)
...
type FluidEmitter2dDispatcher () =
        ...
        [define Entity.FluidParticleRenders
            (Map.ofList
                [("Water", { Transform = Transform.makeIntuitive false v3Zero v3One v3Zero fluidSize v3Zero -0.03f perimeterCentered; InsetOpt = ValueNone; ClipOpt = ValueNone; Image = Assets.Default.Fluid; Color = colorPacked 0x0094FFFFu; Blend = Transparent; Emission = colorZero; Flip = Unflipped })
                 ...
                 ("Smoke", { Transform = Transform.makeIntuitive false v3Zero v3One v3Zero smokeSize v3Zero 0.0f perimeterCentered; InsetOpt = ValueNone; ClipOpt = ValueNone; Image = Assets.Default.Gas; Color = colorOne; Blend = Transparent; Emission = colorZero; Flip = Unflipped })
    ...
    override this.Render (_, emitter, world) =
        ...
        for particle in emitter.GetFluidParticles world do
            ...
            let mutable transform = Transform.makeIntuitive false (render.Transform.Position + particle.FluidParticlePosition) (render.Transform.Scale * transform.Scale) (render.Transform.Offset + transform.Offset) render.Transform.Size (render.Transform.Angles + transform.Angles) (render.Transform.Elevation + transform.Elevation) (emitter.GetPerimeterCentered world)
