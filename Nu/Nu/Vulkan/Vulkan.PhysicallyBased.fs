...
module PhysicallyBased =
    ...
    let BeginPhysicallyBasedForwardPipeline
        ...
        if drawIndex < pipeline.Pipeline.DrawLimit then
            ...
            for i in drawIndex .. dec (min (drawIndex + drawCount) pipeline.Pipeline.DrawLimit) do
                ...
                Pipeline.Pipeline.writeDescriptorSampledImage 0 i 0 2 depthTexture pipeline.Pipeline vkc
                Pipeline.Pipeline.writeDescriptorSampledImage 0 i 0 3 colorTexture pipeline.Pipeline vkc
                Pipeline.Pipeline.writeDescriptorSampledImage 0 i 0 4 brdfTexture pipeline.Pipeline vkc
                Pipeline.Pipeline.writeDescriptorSampledImage 0 i 0 5 irradianceMap pipeline.Pipeline vkc
                Pipeline.Pipeline.writeDescriptorSampledImage 0 i 0 6 environmentFilterMap pipeline.Pipeline vkc
    ...
    let DrawPhysicallyBasedForwardSurfaces
        ...
        if surfacesCount > 0 && drawIndex < pipeline.Pipeline.DrawLimit then
            ...
            Pipeline.Pipeline.writeDescriptorSampledImage 0 drawIndex 1 5 material.AlbedoTexture pipeline.Pipeline vkc
            Pipeline.Pipeline.writeDescriptorSampledImage 0 drawIndex 1 6 material.RoughnessTexture pipeline.Pipeline vkc
            Pipeline.Pipeline.writeDescriptorSampledImage 0 drawIndex 1 7 material.MetallicTexture pipeline.Pipeline vkc
            Pipeline.Pipeline.writeDescriptorSampledImage 0 drawIndex 1 8 material.AmbientOcclusionTexture pipeline.Pipeline vkc
            Pipeline.Pipeline.writeDescriptorSampledImage 0 drawIndex 1 9 material.EmissionTexture pipeline.Pipeline vkc
            Pipeline.Pipeline.writeDescriptorSampledImage 0 drawIndex 1 10 material.NormalTexture pipeline.Pipeline vkc
            Pipeline.Pipeline.writeDescriptorSampledImage 0 drawIndex 1 11 material.HeightTexture pipeline.Pipeline vkc
            for i in 0 .. dec (min irradianceMaps.Length Constants.Render.LightMapsMaxForward) do
                Pipeline.Pipeline.writeDescriptorSampledImage 0 (drawIndex * Constants.Render.LightMapsMaxForward + i) 1 18 irradianceMaps.[i] pipeline.Pipeline vkc
            for i in 0 .. dec (min environmentFilterMaps.Length Constants.Render.LightMapsMaxForward) do
                Pipeline.Pipeline.writeDescriptorSampledImage 0 (drawIndex * Constants.Render.LightMapsMaxForward + i) 1 19 environmentFilterMaps.[i] pipeline.Pipeline vkc
            Pipeline.Pipeline.writeDescriptorSampledImage 0 drawIndex 1 20 shadowTextureArray pipeline.Pipeline vkc
            for i in 0 .. dec (min shadowMaps.Length Constants.Render.ShadowMapsMax) do
                Pipeline.Pipeline.writeDescriptorSampledImage 0 (drawIndex * Constants.Render.ShadowMapsMax + i) 1 21 shadowMaps.[i] pipeline.Pipeline vkc
            for i in 0 .. dec (min shadowCascades.Length Constants.Render.ShadowCascadesMax) do
                Pipeline.Pipeline.writeDescriptorSampledImage 0 (drawIndex * Constants.Render.ShadowCascadesMax + i) 1 22 shadowCascades.[i] pipeline.Pipeline vkc
