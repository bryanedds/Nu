...
module Vulkan =
    ...
    let hlCreateBuffer<'ub when 'ub : unmanaged> (allocator : VmaAllocator, usage : VkBufferUsageFlags, buffer : VkBuffer byref, allocation : VmaAllocation byref) =
        let size = sizeof<'ub>
    ...
    let hlWriteBuffer<'ub when 'ub : unmanaged> (allocator : VmaAllocator, allocation : VmaAllocation, values : 'ub inref) =
