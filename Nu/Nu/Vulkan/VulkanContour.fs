...
module Contour =
    ...
    let createPipeline vkc =
        ...
        let quadVertexBuffer = Buffer.create (Vertex true) (sizeof<Vector2> * ContourQuad.vertices.Length) vkc
        let quadIndexBuffer = Buffer.create (BufferType.Index true) (sizeof<uint32> * ContourQuad.indices.Length) vkc

        Buffer.uploadArray ContourQuad.vertices quadVertexBuffer vkc
        Buffer.uploadArray ContourQuad.indices quadIndexBuffer vkc

        let shapeDataBuffer = Buffer.create Uniform (sizeof<ShapeGPU> * initialShapeDataSize) vkc
        let curveDataBuffer = Buffer.create Storage (sizeof<Vector4> * 2 * maxCurves) vkc
        let bandDataBuffer = Buffer.create Storage (sizeof<uint32> * initialBandDataSize) vkc
    ...
    let private drawContourSlugGeometry
        ...
        (viewport : Viewport)
        ((quadVertexBuffer, quadIndexBuffer, shapeDataBuffer, curveDataBuffer, bandDataBuffer, pipeline) :
            Nu.Vulkan.Buffer * Nu.Vulkan.Buffer * Nu.Vulkan.Buffer * Nu.Vulkan.Buffer * Nu.Vulkan.Buffer * Pipeline)
        (vkc : VulkanContext) =
        ...
        if geometry.Curves.Length > 0 then
            ...
            if geometry.BandEntries.Length <> geometry.HBands + geometry.VBands then
                ...
                | Some vkPipeline ->
                    ...
                    Buffer.uploadArray packedCurves curveDataBuffer vkc
                    ...
                    Buffer.uploadArray bandDataPacked bandDataBuffer vkc
                    ...
                    Buffer.uploadValue shapeGPU shapeDataBuffer vkc
                    ...
                    let mutable uniformDescriptorSet = Pipeline.specifyDescriptorSet 0 pipeline.DrawIndex pipeline vkc $ fun vkSet ->
                        Pipeline.writeDescriptorUniformBuffer 0 0 shapeDataBuffer vkSet vkc
                        Pipeline.writeDescriptorStorageBuffer 1 0 curveDataBuffer vkSet vkc
                        Pipeline.writeDescriptorStorageBuffer 2 0 bandDataBuffer vkSet vkc
                    ...
                    if Hl.validateRect scissor then
                        ...
                        Vulkan.vkCmdBindIndexBuffer (vkc.RenderCommandBuffer, quadIndexBuffer.VkBuffer, 0UL, VkIndexType.Uint32)
    ...
    let drawContour
        ...
        (clipOpt : Box2 voption inref)
        (viewport : Viewport)
        (buffers : Nu.Vulkan.Buffer * Nu.Vulkan.Buffer * Nu.Vulkan.Buffer * Nu.Vulkan.Buffer * Nu.Vulkan.Buffer * Pipeline)
        (vkc : VulkanContext) =
