...
module FastMethod =
    ...
    [<AbstractClass>]
    type private Factory<'T>() =
        inherit Factory()
        abstract Prepare : 'T -> FastMethod
        override this.Prepare(x: obj) = this.Prepare(x :?> 'T)

    [<Sealed>]
    type private Func0<'R>() =
        inherit Factory<F<'R>>()
        override this.Prepare(d: F<'R>) = { new FastMethod() with override this.Invoke0() = box (d.Invoke()) }

    [<Sealed>]
    type private Func1<'T1,'R>() =
        inherit Factory<F<'T1,'R>>()
        override this.Prepare(d: F<'T1,'R>) = { new FastMethod() with override this.Invoke1(x1) = box (d.Invoke(!x1)) }

    [<Sealed>]
    type private Func2<'T1,'T2,'R>() =
        inherit Factory<F<'T1,'T2,'R>>()
        override this.Prepare(d: F<'T1,'T2,'R>) = { new FastMethod() with override this.Invoke2(x1, x2) = box (d.Invoke(!x1, !x2)) }

    [<Sealed>]
    type private Func3<'T1,'T2,'T3,'R>() =
        inherit Factory<F<'T1,'T2,'T3,'R>>()
        override this.Prepare(d: F<'T1,'T2,'T3,'R>) = { new FastMethod() with override this.Invoke3(x1, x2, x3) = box (d.Invoke(!x1, !x2, !x3)) }

    [<Sealed>]
    type private Func4<'T1,'T2,'T3,'T4,'R>() =
        inherit Factory<F<'T1,'T2,'T3,'T4,'R>>()
        override this.Prepare(d: F<'T1,'T2,'T3,'T4,'R>) = { new FastMethod() with override this.Invoke4(x1, x2, x3, x4) = box (d.Invoke(!x1, !x2, !x3, !x4)) }

    [<Sealed>]
    type private Func5<'T1,'T2,'T3,'T4,'T5,'R>() =
        inherit Factory<F<'T1,'T2,'T3,'T4,'T5,'R>>()
        override this.Prepare(d: F<'T1,'T2,'T3,'T4,'T5,'R>) = { new FastMethod() with override this.Invoke5(x1, x2, x3, x4, x5) = box (d.Invoke(!x1, !x2, !x3, !x4, !x5)) }

    [<Sealed>]
    type private Func6<'T1,'T2,'T3,'T4,'T5,'T6,'R>() =
        inherit Factory<F<'T1,'T2,'T3,'T4,'T5,'T6,'R>>()
        override this.Prepare(d: F<'T1,'T2,'T3,'T4,'T5,'T6,'R>) = { new FastMethod() with override this.Invoke6(x1, x2, x3, x4, x5, x6) = box (d.Invoke(!x1, !x2, !x3, !x4, !x5, !x6)) }

    [<Sealed>]
    type private Func7<'T1,'T2,'T3,'T4,'T5,'T6,'T7,'R>() =
        inherit Factory<F<'T1,'T2,'T3,'T4,'T5,'T6,'T7,'R>>()
        override this.Prepare(d: F<'T1,'T2,'T3,'T4,'T5,'T6,'T7,'R>) = { new FastMethod() with override this.Invoke7(x1, x2, x3, x4, x5, x6, x7) = box (d.Invoke(!x1, !x2, !x3, !x4, !x5, !x6, !x7)) }

    [<Sealed>]
    type private Act0() =
        inherit Factory<A>()
        override this.Prepare(d: A) = { new FastMethod() with override this.Invoke0() = d.Invoke(); null }

    [<Sealed>]
    type private Act1<'T1>() =
        inherit Factory<A<'T1>>()
        override this.Prepare(d: A<'T1>) = { new FastMethod() with override this.Invoke1(x1) = d.Invoke(!x1); null }

    [<Sealed>]
    type private Act2<'T1,'T2>() =
        inherit Factory<A<'T1,'T2>>()
        override this.Prepare(d: A<'T1,'T2>) = { new FastMethod() with override this.Invoke2(x1, x2) = d.Invoke(!x1, !x2); null }

    [<Sealed>]
    type private Act3<'T1,'T2,'T3>() =
        inherit Factory<A<'T1,'T2,'T3>>()
        override this.Prepare(d: A<'T1,'T2,'T3>) = { new FastMethod() with override this.Invoke3(x1, x2, x3) = d.Invoke(!x1, !x2, !x3); null }

    [<Sealed>]
    type private Act4<'T1,'T2,'T3,'T4>() =
        inherit Factory<A<'T1,'T2,'T3,'T4>>()
        override this.Prepare(d: A<'T1,'T2,'T3,'T4>) = { new FastMethod() with override this.Invoke4(x1, x2, x3, x4) = d.Invoke(!x1, !x2, !x3, !x4); null }

    [<Sealed>]
    type private Act5<'T1,'T2,'T3,'T4,'T5>() =
        inherit Factory<A<'T1,'T2,'T3,'T4,'T5>>()
        override this.Prepare(d: A<'T1,'T2,'T3,'T4,'T5>) = { new FastMethod() with override this.Invoke5(x1, x2, x3, x4, x5) = d.Invoke(!x1, !x2, !x3, !x4, !x5); null }

    [<Sealed>]
    type private Act6<'T1,'T2,'T3,'T4,'T5,'T6>() =
        inherit Factory<A<'T1,'T2,'T3,'T4,'T5,'T6>>()
        override this.Prepare(d: A<'T1,'T2,'T3,'T4,'T5,'T6>) = { new FastMethod() with override this.Invoke6(x1, x2, x3, x4, x5, x6) = d.Invoke(!x1, !x2, !x3, !x4, !x5, !x6); null }

    [<Sealed>]
    type private Act7<'T1,'T2,'T3,'T4,'T5,'T6,'T7>() =
        inherit Factory<A<'T1,'T2,'T3,'T4,'T5,'T6,'T7>>()
        override this.Prepare(d: A<'T1,'T2,'T3,'T4,'T5,'T6,'T7>) = { new FastMethod() with override this.Invoke7(x1, x2, x3, x4, x5, x6, x7) = d.Invoke(!x1, !x2, !x3, !x4, !x5, !x6, !x7); null }

    /// Compiles a method to a fast invoke function.
    let compile (info: MethodInfo): FastMethod =
        let ts =
            [| if not info.IsStatic then yield info.DeclaringType
               for p in info.GetParameters() do yield p.ParameterType |]
