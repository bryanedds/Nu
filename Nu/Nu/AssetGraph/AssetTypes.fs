...
module TmxExtensions =
    ...
    type DotTiled.Tileset with
        /// Get the associated image asset for the given TileSet in the specified package.
        /// If the TileSet object has a custom property named "Image" with an asset value like "[PackageName AssetName]",
        ...
        /// property "Image" is not decodable to an asset value or the tile set's image source filename cannot be inferred.
        /// Thread-safe.
        member this.GetImageAsset tileMapPackage =
            match this.TryGetProperty "Image" with
            | (true, imageAssetTagString : DotTiled.StringProperty) ->
                try scvalueMemo<Image AssetTag> imageAssetTagString.Value
                with :? KeyNotFoundException ->
                    raise (TileSetPropertyNotFoundException $"Tileset '{this.Name}' has a custom Image property but \
                        cannot be decoded as an asset value like '[PackageName AssetName]'.")
            | _ when this.Image.HasValue ->
                let name = PathF.GetFileNameWithoutExtension this.Image.Value.Source
                asset tileMapPackage name // infer asset tag
            | _ ->
                raise (TileSetPropertyNotFoundException $"Tileset '{this.Name}' does not have custom Image property and is not based on a single image.\n\
    ...
    type DotTiled.Map with
        ...
        member this.GetImageAssets tileMapPackage =
            this.Tilesets.ToArray ()
            |> Array.map (fun tileSet -> struct (tileSet, tileSet.GetImageAsset tileMapPackage))