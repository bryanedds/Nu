                ...
                let texture =
                    if is2d then
                        let (metadata, vulkanTexture) =
                            ...
                            then Texture.CreateTextureVulkanFromData (Vulkan.VK_FILTER_LINEAR, Vulkan.VK_FILTER_LINEAR, true, true, Texture.Uncompressed, textureData, Texture.RenderThread, vkc)
                            else Texture.CreateTextureVulkanFromData (Vulkan.VK_FILTER_NEAREST, Vulkan.VK_FILTER_NEAREST, false, false, Texture.Uncompressed, textureData, Texture.RenderThread, vkc)
                        Texture.EagerTexture { TextureMetadata = metadata; VulkanTexture = vulkanTexture }
                    elif textureData.LazyLoadable then
                        let (metadata, vulkanTexture) = Texture.CreateTextureVulkanFromData (Vulkan.VK_FILTER_LINEAR, Vulkan.VK_FILTER_LINEAR, true, true, Texture.InferCompression filePath, textureData, Texture.RenderThread, vkc)
                        let lazyTexture = new Texture.LazyTexture (filePath, metadata, vulkanTexture, Vulkan.VK_FILTER_LINEAR, Vulkan.VK_FILTER_LINEAR, true)
