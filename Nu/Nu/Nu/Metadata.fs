...
module Metadata =
    ...
    let private generateTextureMetadata asset =
        if not <| File.Exists asset.FilePath then
            ...
            with _ as exn ->
                let errorMessage = "Failed to load Bitmap '" + asset.FilePath + "' due to '" + xstring exn + "'."
                trace errorMessage
                InvalidMetadata errorMessage
    ...
    let private generateTileMapMetadata asset =
        ...
        with _ as exn ->
            let errorMessage = "Failed to load TmxMap '" + asset.FilePath + "' due to '" + xstring exn + "'."
            trace errorMessage
            InvalidMetadata errorMessage
    ...
    let private generateAssetMetadata asset =
        ...
        let metadata =
            ...
            | ".wav" -> SoundMetadata
            | ".ogg" -> SongMetadata
            | _ -> InvalidMetadata <| "Could not load asset metadata '" + xstring asset + "' due to unknown extension '" + extension + "'."
        (asset.Name, metadata)
