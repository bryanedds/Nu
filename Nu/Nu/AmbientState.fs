...
module AmbientStateModule =
    ...
    module AmbientState =
        ...
        let restoreTasklets tasklets state =
            { state with Tasklets = UList.makeFromSeq (UList.getConfig state.Tasklets) (Seq.append (state.Tasklets :> _ seq) (tasklets :> _ seq)) }
        ...
        let make tickRate assetMetadataMap overlayRouter overlayer symbolStore userState =
            { TickRate = tickRate
              ...
              UpdateCount = 0L
              Liveness = Running
              Tasklets = UList.makeEmpty Functional
