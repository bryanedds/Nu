...
module Render =
    ...
    let [<Uniform>] SsaoResolutionX = ResolutionX / 2
    let [<Uniform>] SsaoResolutionY = ResolutionY / 2
    ...
    let [<Uniform>] WindowClearColor = Color.White // NOTE: do not change this color as the deferred lighting shader checks if normal color is equal to [1;1;1] to discard fragment.
