...
module Render =
    ...
    let [<Uniform>] mutable ShadowTexturesMax = match ConfigurationManager.AppSettings.["ShadowTexturesMax"] with null -> 12 | shadowTexturesMax -> min (scvalue shadowTexturesMax) ShadowTexturesMaxShader
    let [<Uniform>] mutable ShadowMapsMax = match ConfigurationManager.AppSettings.["ShadowMapsMax"] with null -> 6 | shadowMapsMax -> min (scvalue shadowMapsMax) ShadowMapsMaxShader
