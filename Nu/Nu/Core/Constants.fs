...
module Render =
    ...
    let [<Uniform>] mutable FarPlaneDistanceEnclosed = match ConfigurationManager.AppSettings.["FarPlaneDistanceEnclosed"] with null -> 16.0f  | scalar -> scvalue<single> scalar // NOTE: remember to update OPAQUING_DISTANCE in shader when changing this!
    ...
    let [<Uniform>] mutable FarPlaneDistanceImposter = match ConfigurationManager.AppSettings.["FarPlaneDistanceImposter"] with null -> 16384.0f | scalar -> scvalue<single> scalar // TODO: see if this being large makes SSAO rather less accurate.
