...
module Engine =
    ...
    let (*Literal*) DesiredFps = Fps60
    let (*Literal*) DesiredFpsI = match DesiredFps with Fps30 -> 30 | Fps60 -> 60
    let (*Literal*) DesiredFpsF = match DesiredFps with Fps30 -> 30.0f | Fps60 -> 60.0f
    ...
    let (*Literal*) EntityCentered2dDefault = match ConfigurationManager.AppSettings.["EntityCentered2dDefault"] with null -> true | centered -> scvalue<bool> centered
    let (*Literal*) EntityCenteredGuiDefault = match ConfigurationManager.AppSettings.["EntityCenteredGuiDefault"] with null -> true | centered -> scvalue<bool> centered
    let (*Literal*) EntityCentered3dDefault = match ConfigurationManager.AppSettings.["EntityCentered3dDefault"] with null -> true | centered -> scvalue<bool> centered
    ...
    let (*Literal*) EyeCenter3dDefault = Vector3 (0.0f, 1.0f, 4.0f)
    let (*Literal*) EyeCenter3dOffset = Vector3 (0.0f, 0.0f, 4.0f)
    ...
    let (*Literal*) EventTracing = match ConfigurationManager.AppSettings.["EventTracing"] with null -> false | tracing -> scvalue<bool> tracing
    let (*Literal*) EventFilter = match ConfigurationManager.AppSettings.["EventFilter"] with null -> EventFilter.Empty | filter -> scvalue<EventFilter.Filter> filter
...
module Render =
    ...
    let (*Literal*) VirtualScalar = match ConfigurationManager.AppSettings.["VirtualScalar"] with null -> 2 | scalar -> scvalue<int> scalar
...
module Physics =
    ...
    let (*Literal*) PhysicsStepRate = 1.0f / match Engine.DesiredFps with Fps30 -> 30.0f | Fps60 -> 60.0f
