...
module Render =
    ...
    let [<Uniform>] ShadowCascadeLimits = match ConfigurationManager.AppSettings.["ShadowCascadeLimits"] with null -> [|0.1f; 0.7f; 1.0f|] | value -> scvalue value
    let [<Uniform>] ShadowCascadeSegmentOverflow = match ConfigurationManager.AppSettings.["ShadowCascadeOverflow"] with null -> 1.5f | value -> scvalue value // TODO: P0: place this in lighting config?
