...
module WorldTests =
    ...
    let [<Fact>] iterativeFrpWorks () =
        let world = World.makeDefault ()
        let world = World.createEntity (Some Jim.Names) DefaultOverlay Simulants.DefaultGroup world |> snd
        let world = World.createEntity (Some Bob.Names) DefaultOverlay Simulants.DefaultGroup world |> snd
