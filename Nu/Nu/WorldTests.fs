...
module WorldTests =
    ...
    let [<Fact>] iterativeFrpWorks () =
        let world = World.makeDefault ()
        let world = World.createEntity (Some Jim.EntityNames) DefaultOverlay Simulants.DefaultGroup world |> snd
        let world = World.createEntity (Some Bob.EntityNames) DefaultOverlay Simulants.DefaultGroup world |> snd
