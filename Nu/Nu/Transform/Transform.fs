...
module TransformMasks =
    ...
    let [<Literal>] ClippedMask =                   0b000000000000010000000u
    ...
    let [<Literal>] FlagsDefault =                  0b100110100000101110001u
...
type [<NoEquality; NoComparison>] Transform =
    ...
    member this.Clipped                 with get () = this.Flags_ &&& ClippedMask <> 0u                 and set value = this.Flags_ <- if value then this.Flags_ ||| ClippedMask else this.Flags_ &&& ~~~ClippedMask
