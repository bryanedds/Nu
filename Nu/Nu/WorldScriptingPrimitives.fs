...
module WorldScriptingPrimitives =
    ...
    module Scripting =
        ...
        let evalV2 fnName originOpt evaledArg evaledArg2 world =
            ...
            | (_, _) -> (Violation (["InvalidArgumentType"; String.capitalize fnName], "Application of " + fnName + " requires a single for the both arguments.", originOpt), world)

        let evalNOf fnName originOpt evaledArg evaledArg2 world =
            match (evaledArg, evaledArg2) with
            | (Single x, Single y) -> (Vector2 (OpenTK.Vector2 (x, y)), world)
            | (Violation _ as violation, _) -> (violation, world)
            | (_, (Violation _ as violation)) -> (violation, world)
        ...
        let evalNth5 index fnName originOpt evaledArg world =
            ...
            | Vector2 v2 ->
                ...
                | 0 -> (Single v2.X, world)
                | 1 -> (Single v2.Y, world)
                | _ -> (Violation (["OutOfRangeArgument"; String.capitalize fnName], "Vector2 does not contain element at index " + string index + ".", originOpt), world)
            | Tuple arr ->
        ...
        let rec evalMapCodata evalApply originOpt mapper codata (world : World) : Codata * World =
            match codata with
            | Empty ->
                (codata, world)
            | Add (left, right) ->
        ...
        let evalMonitor6 fnName originOpt evaledArg evaledArg2 world =
            match evaledArg with
            | Binding _
            | Fun _ ->
        ...
        let evalMonitor fnName originOpt evaledArg evaledArg2 world =
            evalMonitor6 fnName originOpt evaledArg evaledArg2 world