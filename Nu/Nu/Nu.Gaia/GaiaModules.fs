...
module Gaia =
    ...
    let private handleNuEntityAdd (form : GaiaForm) evt world =
        addTreeViewNode form (Entity.proxy ^ atoa evt.Publisher.ObjAddress) world
        (Cascade, world)

    let private handleNuEntityRemoving (form : GaiaForm) evt world =
        match form.treeView.Nodes.Find (symstring evt.Publisher.ObjAddress, true) with
        ...
        | null -> (Cascade, world)
        | :? EntityTypeDescriptorSource as entityTds ->
            if atoa evt.Publisher.ObjAddress = entityTds.DescribedEntity.EntityAddress then
