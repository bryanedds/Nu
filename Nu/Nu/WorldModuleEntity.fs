...
module WorldModuleEntity =
    ...
    type World with
        ...
        static member internal setEntityCreationTimeStamp value entity world = World.updateEntityStatePlus (fun entityState -> if value <> entityState.CreationTimeStamp then (let entityState = if entityState.Imperative then entityState else EntityState.diverge entityState in entityState.CreationTimeStamp <- value; entityState) else Unchecked.defaultof<_>) Property? CreationTimeStamp value entity world
        ...
        static member duplicateEntity source (destination : Entity) world =
            ...
            | null -> world
            | _ ->
                let entityState = { entityStateOpt with CreationTimeStamp = Core.getUniqueTimeStamp (); Id = Gen.id; Names = destination.Names }
        ...
        static member reassignEntityImmediate entity namesOpt (group : Group) world =
            ...
            let entityState = { entityState with CreationTimeStamp = Core.getUniqueTimeStamp (); Id = id; Names = names }
